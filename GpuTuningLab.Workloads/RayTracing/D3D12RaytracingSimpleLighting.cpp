...
void D3D12RaytracingSimpleLighting::ParseCommandLineArgs(WCHAR* argv[], int argc)
{
    ...
    auto parseBoundedInteger = [&](int& index, int minimum, int maximum, const char* argumentName)
    {
        if (index + 1 >= argc)
        {
            throw std::invalid_argument(std::string(argumentName) + " requires a value.");
        }

        wchar_t* end = nullptr;
        long value = wcstol(argv[index + 1], &end, 10);
        if (end == argv[index + 1] || *end != L'\0' || value < minimum || value > maximum)
        {
            std::ostringstream message;
            message << argumentName << " must be an integer from " << minimum << " to " << maximum << ".";
    ...
    {
        if (_wcsicmp(argv[i], L"--seconds") == 0)
        {
            m_measurementSeconds = parseBoundedInteger(i, 2, 120, "--seconds");
        }
        else if (_wcsicmp(argv[i], L"--warmup") == 0)
        {
            m_warmupSeconds = parseBoundedInteger(i, 0, 30, "--warmup");
        }
    }
}
...
void D3D12RaytracingSimpleLighting::OnInit()
{
    ...
    status << "Workload: DXR primary rays | " << m_width << "x" << m_height
        << " | warmup " << m_warmupSeconds << " s"
        << " | measure " << m_measurementSeconds << " s"
        << " | adapter " << BenchmarkOutput::Utf8(m_deviceResources->GetAdapterDescription());
    BenchmarkOutput::WriteLine(status.str());
}
...
{
    if (m_finished || !m_deviceResources->IsWindowVisible())
    {
        return;
    }
    ...
    {
        double warmupElapsed = std::chrono::duration<double>(now - m_warmupStartedAt).count();
        if (warmupElapsed >= static_cast<double>(m_warmupSeconds))
        {
            m_deviceResources->WaitForGpu();
            m_measurementStarted = true;
            m_measurementStartedAt = std::chrono::steady_clock::now();
            m_measuredFrames = 0;
            BenchmarkOutput::WriteLine("Measurement started.");
        }
        return;
    }

    m_measuredFrames++;
    double measuredElapsed = std::chrono::duration<double>(now - m_measurementStartedAt).count();
    if (measuredElapsed >= static_cast<double>(m_measurementSeconds))
    {
        CompleteMeasurement();
    }
}

void D3D12RaytracingSimpleLighting::CompleteMeasurement()
{
    m_deviceResources->WaitForGpu();
    auto completedAt = std::chrono::steady_clock::now();
    double elapsedSeconds = std::chrono::duration<double>(completedAt - m_measurementStartedAt).count();
    double primaryRays = static_cast<double>(m_width) * static_cast<double>(m_height) * static_cast<double>(m_measuredFrames);
    double millionPrimaryRaysPerSecond = primaryRays / elapsedSeconds / 1e6;

    ThrowIfFalse(m_measuredFrames > 0 && elapsedSeconds > 0.0 && std::isfinite(millionPrimaryRaysPerSecond),
        L"The ray tracing workload did not produce a valid measurement.");

    std::ostringstream result;
    result << std::fixed << std::setprecision(3)
        << "Final score: " << millionPrimaryRaysPerSecond << " M primary-rays/s"
        << " | frames: " << m_measuredFrames
        << " | elapsed: " << elapsedSeconds << " s";
    BenchmarkOutput::WriteLine(result.str());
